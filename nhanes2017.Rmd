...
```
Here to reference my LCsum and DPQ sum linear regression plot. Figure\\@ref(fig:DPQgenderdiff-plot)
```{r LCDPQ-plot}
#My second plot would be a linear regression 
library(ggplot2)
LCsumnotzero_data <- fix_nhanes_data[fix_nhanes_data$LCsum != 0, ]
...
# Print the table
rac_table <- kable(racial_demographics,col.names = c("race","count"),align = "c", caption = "Racial Demographics in nhanes_data")

#Do it apa style, having some problem.
apa_table(rac_table)
