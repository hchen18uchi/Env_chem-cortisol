...
To assess the relationship between these ordinal variables, we employed Spearman's Rank Correlation coefficient test. This non-parametric test was chosen due to the ordinal nature of both LCsum and DPQsum, offering a robust measure of the monotonic relationship between trace metal exposure and depression symptom frequency. The Spearman correlation analysis extends beyond the aggregate LCsum, as we also systematically explored the correlation between each individual trace metal concentration and DPQsum scores. This comprehensive approach allows us to discern specific trace metals that may have a more pronounced association with depression symptoms, thereby contributing to our understanding of potential environmental risk factors influencing mental health.
...
#r=0.0303 p=0.191
```
The Spearman test result for the type of trace metal exceed detection limits and the frequency of depression symptom score shows a correlation coefficient (rho) of `r round(spearman_test_LCsum$estimate, 4)` with a p-value of `r round(spearman_test_LCsum$p.value, 4)`.

```{r test-result-table, echo=FALSE}
...
```
Means of LCsum is `r mean_LCsum`, which means participants have an average of 10 types of trace metal above detection limits. For the same people group, their average score for frequency of depression symptom is `r mean_DPQsum`. 
